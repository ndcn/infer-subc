...
from skimage.filters import threshold_triangle, threshold_otsu, threshold_li, threshold_multiotsu
...
def fill_labeled_holes(labels, mask=None, size_fn=None):
    ...
    * there is no path from the pixel to two similarly-labeled pixels that
    ...
    #
    to_do = np.unique(np.hstack((labels[0, :], labels[:, 0], labels[-1, :], labels[:, -1])))
    to_do = to_do[to_do != 0]
    ...
    if (len(i)) > 0:
        ...
        if size_fn is not None:
            ...
            for ii, area in enumerate(areas):
                if ii > 0 and area > 0 and not is_not_hole[ii] and not size_fn(area, ii <= lcount):
                    is_not_hole[ii] = True
                    to_do.append(ii)
...
def get_local_threshold(image, mask, volumetric, adaptive_window_size):
    ...
    if len(image_data) == 0 or np.all(image_data == np.nan):
        ...
        if adaptive_window % 2 == 0:
            adaptive_window += 1
        local_threshold = skimage.filters.threshold_sauvola(image_data, window_size=adaptive_window)
...
def _get_adaptive_threshold(
    ...
    **kwargs,
    ...
    if threshold_operation == TM_OTSU:
        bin_wanted = 0 if assign_middle_to_foreground.value == "Foreground" else 1
...
def cp_adaptive_threshold(image_data, th_method, volumetric, window_size, log_scale):  # skimage.filters.threshold_li,
    """
...
def get_threshold(image, th_guide, th_original, automatic=False, log_scale=False):
